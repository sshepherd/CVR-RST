...
/* rngoff argument in ACFCalculate.. is 2*rxchn and is normally set to 2 */
  rxchn=1;
  if (debug) {
    fprintf(stderr,"SiteCvwStart: rxchn=%d\n",rxchn);
  } 
  return 0;
}
...
int SiteCvwStartIntt(int sec,int usec) {
  ...
  SiteCvwExit(0);
  if (debug) {
    fprintf(stderr,"SiteCvwStartInt: start\n");
  }
  ...
  if (debug) {
    fprintf(stderr,"SiteCvwStartInt: end\n");
  }
  return 0;
...
*/
    ...
    if(dprm.status==0) {
      ...
      if (debug) 
        fprintf(stderr,"CVW GET_DATA: badtrdat.start_usec: uint32: %d array: %d\n",sizeof(uint32),sizeof(uint32)*badtrdat.length);
...
/* total_samples*8 represents number of bytes for main and back samples */
    ...
    /* calculate ACF */   
      if (mplgexs==0) {
        ...
        rngoff=2*rxchn; 
        if (debug) 
        fprintf(stderr,"CVW seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"CVW seq %d :: ACFSumPower\n",nave);
        ...
        fprintf(stderr,"CVW seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"CVW seq %d :: ACFCalculate acf\n",nave);
        ...
        fprintf(stderr,"CVW seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"CVW seq %d :: ACFCalculate xcf\n",nave);
          ACFCalculate(&tsgprm,(int16 *) dest,rngoff,skpnum!=0,
        ...
        fprintf(stderr,"CVW seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"CVW seq %d :: ACFNormalize\n",nave);
...
*/
   ...
   free(lagtable[1]);
   if (debug) {
     fprintf(stderr,"CVW SiteIntegrate: iqsize in bytes: %d in 16bit samples:  %d in 32bit samples: %d\n",iqsze,iqsze/2,iqsze/4);
