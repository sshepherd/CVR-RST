...
int aceswe_plasma(CDFid id,struct plasmadata *ptr,double stime,double etime) {
  ...
  double rtime;
  int i,cnt=0;
  char *varlist[]={"Epoch","V_GSE","V_GSM","Np",NULL};
  struct RCDFData *data=NULL;
  ...
  for (i=0;i<rmax;i++) {
    ...
    if (status==0) {
       ...
       if (fptr[0]<-999) { /* bad data */
           ptr->VGSE[cnt*3]=FILL_VALUE;
           ptr->VGSE[cnt*3+1]=FILL_VALUE;
           ptr->VGSE[cnt*3+2]=FILL_VALUE;
           ptr->VGSM[cnt*3]=FILL_VALUE;
           ptr->VGSM[cnt*3+1]=FILL_VALUE;
           ptr->VGSM[cnt*3+2]=FILL_VALUE;
           ptr->vth[cnt]=FILL_VALUE;
           ptr->den[cnt]=FILL_VALUE;
           ptr->pre[cnt]=FILL_VALUE;
           cnt++;
           continue;
       }
