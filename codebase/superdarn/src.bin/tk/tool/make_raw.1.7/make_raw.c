...
int main (int argc,char *argv[]) {
  ...
  while (IQFread(fp,prm,iq,&badtr,&samples) !=-1) {
   ...
   if (skpval==0) skpval=iq->skpnum;   
    ...
    if (vb) 
      fprintf(stderr,"  beam=%d: pre-ACFBadLagZero\n",prm->bmnum);

    badrng=ACFBadLagZero(&tprm,prm->mplgs,lag);
    
    for (n=0;n<iq->seqnum;n++) {
      if (vb) 
        fprintf(stderr,"  beam=%d: n: %d pre-ACFSumPower\n",prm->bmnum,n);
      ...
      if (vb) {
        fprintf(stderr,"  beam=%d: n: %d pre-ACFCalculate acfd :: \n",prm->bmnum,n);
        fprintf(stderr,"     tprm %p ptr %p rngoff %d dflg %d roff %d ioff %d mplgs %d \n",&tprm,ptr,2*iq->chnnum,skpval !=0,roff,ioff,mplgs);
      }
      ...
      if (vb) 
        fprintf(stderr,"  beam=%d: n: %d pre-ACFCalculate xcfd\n",prm->bmnum,n);
