...
#include "evalefield.h"
#include "crdshft.h"



int CnvMapSolveEfield(struct CnvMapData *ptr,struct CnvGrid *vptr) {
  ...
  CnvMapEvalEfield(ptr->fit_order,ptr->coef,plm,vptr,ptr->latmin,ptr);


  if (ptr->hemisphere==-1)
    for (i=0;i<vptr->num;i++) vptr->ey[i]=-vptr->ey[i];
